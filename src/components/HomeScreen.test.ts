...
describe('TV home presentation', () => {
  it('uses living-room scale cards with a fixed, non-reflowing stride', () => {
    ...
    expect(HOME_FOCUS_WIDTH).toBe(1120)
  ...
  it('pins the focused row high and leaves the next rail visible at TV scale', () => {
    ...
    expect(homeRowTop(3, 2, true)).toBe(934)
    expect(homeRowTop(4, 2, true)).toBe(1454)
  ...
  it('maps regional certification strings to recognizable agency marks', () => {
    expect(certificationPresentation('U')).toEqual({ agency: 'bbfc', label: 'U', className: 'is-bbfc is-u' })
    expect(certificationPresentation('PEGI 16')).toEqual({ agency: 'pegi', label: '16', className: 'is-pegi is-16' })
    expect(certificationPresentation('TV-14')).toEqual({ agency: 'tv', label: 'TV-14', className: 'is-tv' })
  })

  it('enables English captions only for known non-English trailer audio', () => {
    expect(trailerNeedsEnglishCaptions('ja-JP')).toBe(true)
    expect(trailerNeedsEnglishCaptions('jpn')).toBe(true)
    expect(trailerNeedsEnglishCaptions('en')).toBe(false)
    expect(trailerNeedsEnglishCaptions()).toBe(false)
  })
