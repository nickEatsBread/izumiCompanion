    ...
    if (this.cloudflare) {
      void (async () => {
        ...
        try {
          ...
          if (viewer !== tvProfileId() || !tvProfileReady()) return
          // The receiver service can be online without a paired phone or desktop. Account
          // catalogues are cloud-only too, so neither case should wait for a broadcast reply.
