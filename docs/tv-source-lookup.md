...
When cloud playback cannot query a configured source, Companion can request the source
metadata over the TV's home connection and return it to the private Worker.
Izumi can remain closed. Debrid credentials stay in the Worker, which checks
...
contains an opaque ID and a public stream URL. The TV validates the
...
new capability field and retain their previous behavior. Only supported public
requests are accepted; arbitrary credential-bearing URLs are not copied to the TV.

Validation includes receiver routing, credential/URL rejection, disconnect handling,
Worker ticket tampering/expiry/replay checks, and live resolution through the saved debrid account.
The live harness resolved a movie and an episode in about three seconds
